...
result in some cruft.</p> <p><strong>IMPORTANT:</strong> If you previously were passing in a non-tty stream but you still
...
width detection.</p> <p>The <strong>options</strong> object can have the following properties, all of which are
optional:</p> <ul><li><p><strong>updateInterval</strong>: How often gauge updates should be drawn, in milliseconds.</p></li> <li><p><strong>fixedFramerate</strong>: Defaults to false on node 0.8, true on everything
...
expect your program to exit.</p></li> <li><p><strong>themes</strong>: A themeset to use when selecting the theme to use. Defaults
to <code>gauge/themes</code>, see the <a href="#themes">themes</a> documentation for details.</p></li> <li><p><strong>theme</strong>: Select a theme for use, it can be a:</p> <ul><li>Theme object, in which case the <strong>themes</strong> is not used.</li> <li>The name of a theme, which will be looked up in the current <em>themes</em>
object.</li> <li>A configuration object with any of <code>hasUnicode</code>, <code>hasColor</code> or
<code>platform</code> keys, which if will be used to override our guesses when making
a default theme selection.</li></ul> <p>If no theme is selected then a default is picked using a combination of our
best guesses at your OS, color support and unicode support.</p></li> <li><p><strong>template</strong>: Describes what you want your gauge to look like.  The
default is what npm uses.  Detailed <a href="#templates">documentation</a> is later in this
document.</p></li> <li><p><strong>hideCursor</strong>: Defaults to true.  If true, then the cursor will be hidden
while the gauge is displayed.</p></li> <li><p><strong>tty</strong>: The tty that you're ultimately writing to.  Defaults to the same
...
a width of <code>79</code> is assumed.</p></li> <li><p><strong>enabled</strong>: Defaults to true if <code>tty</code> is a TTY, false otherwise.  If true
the gauge starts enabled.  If disabled then all update commands are
ignored and no gauge will be printed until you call <code>.enable()</code>.</p></li> <li><p><strong>Plumbing</strong>: The class to use to actually generate the gauge for
printing.  This defaults to <code>require('gauge/plumbing')</code> and ordinarily you
shouldn't need to override this.</p></li> <li><p><strong>cleanupOnExit</strong>: Defaults to true. Ordinarily we register an exit
...
unexpectedly. You can disable this and it won't register the exit handler.</p></li></ul> <h4 id="gauge-show-section-status-completed"><a href="#gauge-show-section-status-completed" class="header-anchor">#</a> <code>gauge.show(section | status, [completed])</code></h4> <p>The first argument is either the section, the name of the current thing
...
<code>null</code> and it will be ignored.</p> <p>The second argument is the percent completed as a value between 0 and 1.
...
it and the completed argument are passed in, the completed argument wins.</p> <h4 id="gauge-hide-cb"><a href="#gauge-hide-cb" class="header-anchor">#</a> <code>gauge.hide([cb])</code></h4> <p>Removes the gauge from the terminal.  Optionally, callback <code>cb</code> after IO has
had an opportunity to happen (currently this just means after <code>setImmediate</code>
has called back.)</p> <p>It turns out this is important when you're pausing the progress bar on one
...
can cause corruption in some terminals.</p> <h4 id="gauge-pulse-subsection"><a href="#gauge-pulse-subsection" class="header-anchor">#</a> <code>gauge.pulse([subsection])</code></h4> <ul><li><strong>subsection</strong> – <em>(optional)</em> The specific thing that triggered this pulse</li></ul> <p>Spins the spinner in the gauge to show output.  If <strong>subsection</strong> is
included then it will be combined with the last name passed to <code>gauge.show</code>.</p> <h4 id="gauge-disable"><a href="#gauge-disable" class="header-anchor">#</a> <code>gauge.disable()</code></h4> <p>Hides the gauge and ignores further calls to <code>show</code> or <code>pulse</code>.</p> <h4 id="gauge-enable"><a href="#gauge-enable" class="header-anchor">#</a> <code>gauge.enable()</code></h4> <p>Shows the gauge and resumes updating when <code>show</code> or <code>pulse</code> is called.</p> <h4 id="gauge-isenabled"><a href="#gauge-isenabled" class="header-anchor">#</a> <code>gauge.isEnabled()</code></h4> <p>Returns true if the gauge is enabled.</p> <h4 id="gauge-setthemeset-themes"><a href="#gauge-setthemeset-themes" class="header-anchor">#</a> <code>gauge.setThemeset(themes)</code></h4> <p>Change the themeset to select a theme from. The same as the <code>themes</code> option
used in the constructor. The theme will be reselected from this themeset.</p> <h4 id="gauge-settheme-theme"><a href="#gauge-settheme-theme" class="header-anchor">#</a> <code>gauge.setTheme(theme)</code></h4> <p>Change the active theme, will be displayed with the next show or pulse. This can be:</p> <ul><li>Theme object, in which case the <strong>themes</strong> is not used.</li> <li>The name of a theme, which will be looked up in the current <em>themes</em>
object.</li> <li>A configuration object with any of <code>hasUnicode</code>, <code>hasColor</code> or
<code>platform</code> keys, which if will be used to override our guesses when making
a default theme selection.</li></ul> <p>If no theme is selected then a default is picked using a combination of our
best guesses at your OS, color support and unicode support.</p> <h4 id="gauge-settemplate-template"><a href="#gauge-settemplate-template" class="header-anchor">#</a> <code>gauge.setTemplate(template)</code></h4> <p>Change the active template, will be displayed with the next show or pulse</p> <h3 id="tracking-completion"><a href="#tracking-completion" class="header-anchor">#</a> Tracking Completion</h3> <p>If you have more than one thing going on that you want to track completion
of, you may find the related <a href="https://www.npmjs.com/package/are-we-there-yet" target="_blank" rel="noopener noreferrer">are-we-there-yet<span><svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false" x="0px" y="0px" viewBox="0 0 100 100" width="15" height="15" class="icon outbound"><path fill="currentColor" d="M18.8,85.1h56l0,0c2.2,0,4-1.8,4-4v-32h-8v28h-48v-48h28v-8h-32l0,0c-2.2,0-4,1.8-4,4v56C14.8,83.3,16.6,85.1,18.8,85.1z"></path> <polygon fill="currentColor" points="45.7,48.7 51.3,54.3 77.2,28.5 77.2,37.2 85.2,37.2 85.2,14.9 62.8,14.9 62.8,22.9 71.5,22.9"></polygon></svg> <span class="sr-only">(opens new window)</span></span></a> helpful.  It's <code>change</code>
event can be wired up to the <code>show</code> method to get a more traditional
progress bar interface.</p> <h3 id="themes"><a href="#themes" class="header-anchor">#</a> THEMES</h3> <div class="language- extra-class"><pre class="language-text"><code>var themes = require('gauge/themes')
...
</code></pre></div><p>The object returned by <code>gauge/themes</code> is an instance of the <code>ThemeSet</code> class.</p> <div class="language- extra-class"><pre class="language-text"><code>var ThemeSet = require('gauge/theme-set')
var themes = new ThemeSet()
// or
var themes = require('gauge/themes')
var mythemes = themes.newThemeSet() // creates a new themeset based on the default themes
</code></pre></div><h4 id="themes-opts"><a href="#themes-opts" class="header-anchor">#</a> themes(opts)</h4> <h4 id="themes-getdefault-opts"><a href="#themes-getdefault-opts" class="header-anchor">#</a> themes.getDefault(opts)</h4> <p>Theme objects are a function that fetches the default theme based on
platform, unicode and color support.</p> <p>Options is an object with the following properties:</p> <ul><li><strong>hasUnicode</strong> - If true, fetch a unicode theme, if no unicode theme is
available then a non-unicode theme will be used.</li> <li><strong>hasColor</strong> - If true, fetch a color theme, if no color theme is
available a non-color theme will be used.</li> <li><strong>platform</strong> (optional) - Defaults to <code>process.platform</code>.  If no
platform match is available then <code>fallback</code> is used instead.</li></ul> <p>If no compatible theme can be found then an error will be thrown with a
<code>code</code> of <code>EMISSINGTHEME</code>.</p> <h4 id="themes-addtheme-themename-themeobj"><a href="#themes-addtheme-themename-themeobj" class="header-anchor">#</a> themes.addTheme(themeName, themeObj)</h4> <h4 id="themes-addtheme-themename-parenttheme-newtheme"><a href="#themes-addtheme-themename-parenttheme-newtheme" class="header-anchor">#</a> themes.addTheme(themeName, [parentTheme], newTheme)</h4> <p>Adds a named theme to the themeset.  You can pass in either a theme object,
as returned by <code>themes.newTheme</code> or the arguments you'd pass to
<code>themes.newTheme</code>.</p> <h4 id="themes-getthemenames"><a href="#themes-getthemenames" class="header-anchor">#</a> themes.getThemeNames()</h4> <p>Return a list of all of the names of the themes in this themeset. Suitable
for use in <code>themes.getTheme(…)</code>.</p> <h4 id="themes-gettheme-name"><a href="#themes-gettheme-name" class="header-anchor">#</a> themes.getTheme(name)</h4> <p>Returns the theme object from this theme set named <code>name</code>.</p> <p>If <code>name</code> does not exist in this themeset an error will be thrown with
a <code>code</code> of <code>EMISSINGTHEME</code>.</p> <h4 id="themes-setdefault-opts-themename"><a href="#themes-setdefault-opts-themename" class="header-anchor">#</a> themes.setDefault([opts], themeName)</h4> <p><code>opts</code> is an object with the following properties.</p> <ul><li><strong>platform</strong> - Defaults to <code>'fallback'</code>.  If your theme is platform
specific, specify that here with the platform from <code>process.platform</code>, eg,
<code>win32</code>, <code>darwin</code>, etc.</li> <li><strong>hasUnicode</strong> - Defaults to <code>false</code>. If your theme uses unicode you
should set this to true.</li> <li><strong>hasColor</strong> - Defaults to <code>false</code>.  If your theme uses color you should
set this to true.</li></ul> <p><code>themeName</code> is the name of the theme (as given to <code>addTheme</code>) to use for
this set of <code>opts</code>.</p> <h4 id="themes-newtheme-parenttheme-newtheme"><a href="#themes-newtheme-parenttheme-newtheme" class="header-anchor">#</a> themes.newTheme([parentTheme,] newTheme)</h4> <p>Create a new theme object based on <code>parentTheme</code>.  If no <code>parentTheme</code> is
...
fallback parent is defined in <code>gauge/base-theme</code>.)</p> <p>newTheme should be a bare object– we'll start by discussing the properties
defined by the default themes:</p> <ul><li><strong>preProgressbar</strong> - displayed prior to the progress bar, if the progress
bar is displayed.</li> <li><strong>postProgressbar</strong> - displayed after the progress bar, if the progress bar
is displayed.</li> <li><strong>progressBarTheme</strong> - The subtheme passed through to the progress bar
...
bar.</li> <li><strong>activityIndicatorTheme</strong> - The theme for the activity indicator (spinner),
this can either be a string, in which each character is a different step, or
an array of strings.</li> <li><strong>preSubsection</strong> - Displayed as a separator between the <code>section</code> and
<code>subsection</code> when the latter is printed.</li></ul> <p>More generally, themes can have any value that would be a valid value when rendering
templates. The properties in the theme are used when their name matches a type in
the template. Their values can be:</p> <ul><li><strong>strings &amp; numbers</strong> - They'll be included as is</li> <li><strong>function (values, theme, width)</strong> - Should return what you want in your output.
...
and <em>width</em> is the number of characters wide your result should be.</li></ul> <p>There are a couple of special prefixes:</p> <ul><li><strong>pre</strong> - Is shown prior to the property, if its displayed.</li> <li><strong>post</strong> - Is shown after the property, if its displayed.</li></ul> <p>And one special suffix:</p> <ul><li><strong>Theme</strong> - Its value is passed to a function-type item as the theme.</li></ul> <h4 id="themes-addtoallthemes-theme"><a href="#themes-addtoallthemes-theme" class="header-anchor">#</a> themes.addToAllThemes(theme)</h4> <p>This <em>mixes-in</em> <code>theme</code> into all themes currently defined. It also adds it
to the default parent theme for this themeset, so future themes added to
this themeset will get the values from <code>theme</code> by default.</p> <h4 id="themes-newthemeset"><a href="#themes-newthemeset" class="header-anchor">#</a> themes.newThemeSet()</h4> <p>Copy the current themeset into a new one.  This allows you to easily inherit
one themeset from another.</p> <h3 id="templates"><a href="#templates" class="header-anchor">#</a> TEMPLATES</h3> <p>A template is an array of objects and strings that, after being evaluated,
will be turned into the gauge line.  The default template is:</p> <div class="language-javascript extra-class"><pre class="language-javascript"><code><span class="token punctuation">[</span>
    <span class="token punctuation">{</span><span class="token literal-property property">type</span><span class="token operator">:</span> <span class="token string">'progressbar'</span><span class="token punctuation">,</span> <span class="token literal-property property">length</span><span class="token operator">:</span> <span class="token number">20</span><span class="token punctuation">}</span><span class="token punctuation">,</span>
    <span class="token punctuation">{</span><span class="token literal-property property">type</span><span class="token operator">:</span> <span class="token string">'activityIndicator'</span><span class="token punctuation">,</span> <span class="token literal-property property">kerning</span><span class="token operator">:</span> <span class="token number">1</span><span class="token punctuation">,</span> <span class="token literal-property property">length</span><span class="token operator">:</span> <span class="token number">1</span><span class="token punctuation">}</span><span class="token punctuation">,</span>
    <span class="token punctuation">{</span><span class="token literal-property property">type</span><span class="token operator">:</span> <span class="token string">'section'</span><span class="token punctuation">,</span> <span class="token literal-property property">kerning</span><span class="token operator">:</span> <span class="token number">1</span><span class="token punctuation">,</span> <span class="token keyword">default</span><span class="token operator">:</span> <span class="token string">''</span><span class="token punctuation">}</span><span class="token punctuation">,</span>
    <span class="token punctuation">{</span><span class="token literal-property property">type</span><span class="token operator">:</span> <span class="token string">'subsection'</span><span class="token punctuation">,</span> <span class="token literal-property property">kerning</span><span class="token operator">:</span> <span class="token number">1</span><span class="token punctuation">,</span> <span class="token keyword">default</span><span class="token operator">:</span> <span class="token string">''</span><span class="token punctuation">}</span>
<span class="token punctuation">]</span>
</code></pre></div><p>The various template elements can either be <strong>plain strings</strong>, in which case they will
be be included verbatum in the output, or objects with the following properties:</p> <ul><li><em>type</em> can be any of the following plus any keys you pass into <code>gauge.show</code> plus
any keys you have on a custom theme.
<ul><li><code>section</code> – What big thing you're working on now.</li> <li><code>subsection</code> – What component of that thing is currently working.</li> <li><code>activityIndicator</code> – Shows a spinner using the <code>activityIndicatorTheme</code>
from your active theme.</li> <li><code>progressbar</code> – A progress bar representing your current <code>completed</code>
using the <code>progressbarTheme</code> from your active theme.</li></ul></li> <li><em>kerning</em> – Number of spaces that must be between this item and other
items, if this item is displayed at all.</li> <li><em>maxLength</em> – The maximum length for this element. If its value is longer it
will be truncated.</li> <li><em>minLength</em> – The minimum length for this element. If its value is shorter it
will be padded according to the <em>align</em> value.</li> <li><em>align</em> – (Default: left) Possible values &quot;left&quot;, &quot;right&quot; and &quot;center&quot;. Works
as you'd expect from word processors.</li> <li><em>length</em> – Provides a single value for both <em>minLength</em> and <em>maxLength</em>. If both
<em>length</em> and *minLength or <em>maxLength</em> are specified then the latter take precedence.</li> <li><em>value</em> – A literal value to use for this template item.</li> <li><em>default</em> – A default value to use for this template item if a value
wasn't otherwise passed in.</li></ul> <h3 id="plumbing"><a href="#plumbing" class="header-anchor">#</a> PLUMBING</h3> <p>This is the super simple, assume nothing, do no magic internals used by gauge to
implement its ordinary interface.</p> <div class="language- extra-class"><pre class="language-text"><code>var Plumbing = require('gauge/plumbing')
var gauge = new Plumbing(theme, template, width)
</code></pre></div><ul><li><strong>theme</strong>: The theme to use.</li> <li><strong>template</strong>: The template to use.</li> <li><strong>width</strong>: How wide your gauge should be</li></ul> <h4 id="gauge-settheme-theme-2"><a href="#gauge-settheme-theme-2" class="header-anchor">#</a> <code>gauge.setTheme(theme)</code></h4> <p>Change the active theme.</p> <h4 id="gauge-settemplate-template-2"><a href="#gauge-settemplate-template-2" class="header-anchor">#</a> <code>gauge.setTemplate(template)</code></h4> <p>Change the active template.</p> <h4 id="gauge-setwidth-width"><a href="#gauge-setwidth-width" class="header-anchor">#</a> <code>gauge.setWidth(width)</code></h4> <p>Change the width to render at.</p> <h4 id="gauge-hide"><a href="#gauge-hide" class="header-anchor">#</a> <code>gauge.hide()</code></h4> <p>Return the string necessary to hide the progress bar</p> <h4 id="gauge-hidecursor"><a href="#gauge-hidecursor" class="header-anchor">#</a> <code>gauge.hideCursor()</code></h4> <p>Return a string to hide the cursor.</p> <h4 id="gauge-showcursor"><a href="#gauge-showcursor" class="header-anchor">#</a> <code>gauge.showCursor()</code></h4> <p>Return a string to show the cursor.</p> <h4 id="gauge-show-status"><a href="#gauge-show-status" class="header-anchor">#</a> <code>gauge.show(status)</code></h4> <p>Using <code>status</code> for values, render the provided template with the theme and return
a string that is suitable for printing to update the gauge.</p></div> <footer class="page-edit"><!----> <!----></footer> <!----> </main></div><div class="global-ui"><!----></div></div>
    <script src="/edu_db_labs/assets/js/app.b4446a62.js" defer></script><script src="/edu_db_labs/assets/js/2.c940f4cf.js" defer></script><script src="/edu_db_labs/assets/js/1.444b6f58.js" defer></script><script src="/edu_db_labs/assets/js/132.4fae4104.js" defer></script>
  </body>
</html>
