...
<html lang="en-US">
  <head>
    ...
    <title>fs-minipass | Назва проєкту</title>
...
entire file and return the contents.</p> <p>When using <code>WriteStreamSync</code>, every write is flushed to the file
synchronously.  If your writes all come in a single tick, then it'll
write it all out in a single tick.  It's as synchronous as you are.</p> <p>The async versions work much like their node builtin counterparts,
with the exception of introducing significantly less Stream machinery
overhead.</p> <h2 id="usage"><a href="#usage" class="header-anchor">#</a> USAGE</h2> <p>It's just streams, you pipe them or read() them or write() to them.</p> <div class="language-js extra-class"><pre class="language-js"><code><span class="token keyword">const</span> fsm <span class="token operator">=</span> <span class="token function">require</span><span class="token punctuation">(</span><span class="token string">'fs-minipass'</span><span class="token punctuation">)</span>
<span class="token keyword">const</span> readStream <span class="token operator">=</span> <span class="token keyword">new</span> <span class="token class-name">fsm<span class="token punctuation">.</span>ReadStream</span><span class="token punctuation">(</span><span class="token string">'file.txt'</span><span class="token punctuation">)</span>
<span class="token keyword">const</span> writeStream <span class="token operator">=</span> <span class="token keyword">new</span> <span class="token class-name">fsm<span class="token punctuation">.</span>WriteStream</span><span class="token punctuation">(</span><span class="token string">'output.txt'</span><span class="token punctuation">)</span>
writeStream<span class="token punctuation">.</span><span class="token function">write</span><span class="token punctuation">(</span><span class="token string">'some file header or whatever\n'</span><span class="token punctuation">)</span>
...
</code></pre></div><h2 id="readstream-path-options"><a href="#readstream-path-options" class="header-anchor">#</a> ReadStream(path, options)</h2> <p>Path string is required, but somewhat irrelevant if an open file
descriptor is passed in as an option.</p> <p>Options:</p> <ul><li><code>fd</code> Pass in a numeric file descriptor, if the file is already open.</li> <li><code>readSize</code> The size of reads to do, defaults to 16MB</li> <li><code>size</code> The size of the file, if known.  Prevents zero-byte read()
call at the end.</li> <li><code>autoClose</code> Set to <code>false</code> to prevent the file descriptor from being
closed when the file is done being read.</li></ul> <h2 id="writestream-path-options"><a href="#writestream-path-options" class="header-anchor">#</a> WriteStream(path, options)</h2> <p>Path string is required, but somewhat irrelevant if an open file
descriptor is passed in as an option.</p> <p>Options:</p> <ul><li><code>fd</code> Pass in a numeric file descriptor, if the file is already open.</li> <li><code>mode</code> The mode to create the file with. Defaults to <code>0o666</code>.</li> <li><code>start</code> The position in the file to start reading.  If not
specified, then the file will start writing at position zero, and be
truncated by default.</li> <li><code>autoClose</code> Set to <code>false</code> to prevent the file descriptor from being
closed when the stream is ended.</li> <li><code>flags</code> Flags to use when opening the file.  Irrelevant if <code>fd</code> is
passed in, since file won't be opened in that case.  Defaults to
<code>'a'</code> if a <code>pos</code> is specified, or <code>'w'</code> otherwise.</li></ul></div> <footer class="page-edit"><!----> <!----></footer> <!----> </main></div><div class="global-ui"><!----></div></div>
    <script src="/edu_db_labs/assets/js/app.b4446a62.js" defer></script><script src="/edu_db_labs/assets/js/2.c940f4cf.js" defer></script><script src="/edu_db_labs/assets/js/1.444b6f58.js" defer></script><script src="/edu_db_labs/assets/js/126.495bcb43.js" defer></script>
  </body>
</html>
