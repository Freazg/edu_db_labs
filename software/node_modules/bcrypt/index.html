...
<html lang="en-US">
  <head>
    ...
    <title>node.bcrypt.js | Назва проєкту</title>
...
</code></pre></div><h4 id="to-hash-a-password"><a href="#to-hash-a-password" class="header-anchor">#</a> To hash a password:</h4> <p>Technique 1 (generate a salt and hash on separate function calls):</p> <div class="language-javascript extra-class"><pre class="language-javascript"><code>bcrypt<span class="token punctuation">.</span><span class="token function">genSalt</span><span class="token punctuation">(</span>saltRounds<span class="token punctuation">,</span> <span class="token keyword">function</span><span class="token punctuation">(</span><span class="token parameter">err<span class="token punctuation">,</span> salt</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
<span class="token punctuation">}</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
</code></pre></div><p>Technique 2 (auto-gen a salt and hash):</p> <div class="language-javascript extra-class"><pre class="language-javascript"><code>bcrypt<span class="token punctuation">.</span><span class="token function">hash</span><span class="token punctuation">(</span>myPlaintextPassword<span class="token punctuation">,</span> saltRounds<span class="token punctuation">,</span> <span class="token keyword">function</span><span class="token punctuation">(</span><span class="token parameter">err<span class="token punctuation">,</span> hash</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
    <span class="token comment">// Store hash in your password DB.</span>
<span class="token punctuation">}</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
</code></pre></div><p>Note that both techniques achieve the same end-result.</p> <h4 id="to-check-a-password"><a href="#to-check-a-password" class="header-anchor">#</a> To check a password:</h4> <div class="language-javascript extra-class"><pre class="language-javascript"><code><span class="token comment">// Load hash from your password DB.</span>
...
<span class="token punctuation">}</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
</code></pre></div><p><a href="#a-note-on-timing-attacks">A Note on Timing Attacks</a></p> <h3 id="with-promises"><a href="#with-promises" class="header-anchor">#</a> with promises</h3> <p>bcrypt uses whatever <code>Promise</code> implementation is available in <code>global.Promise</code>. NodeJS &gt;= 0.12 has a native <code>Promise</code> implementation built in. However, this should work in any Promises/A+ compliant implementation.</p> <p>Async methods that accept a callback, return a <code>Promise</code> when callback is not specified if Promise support is available.</p> <div class="language-javascript extra-class"><pre class="language-javascript"><code>bcrypt<span class="token punctuation">.</span><span class="token function">hash</span><span class="token punctuation">(</span>myPlaintextPassword<span class="token punctuation">,</span> saltRounds<span class="token punctuation">)</span><span class="token punctuation">.</span><span class="token function">then</span><span class="token punctuation">(</span><span class="token keyword">function</span><span class="token punctuation">(</span><span class="token parameter">hash</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
    <span class="token comment">// Store hash in your password DB.</span>
<span class="token punctuation">}</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
</code></pre></div><div class="language-javascript extra-class"><pre class="language-javascript"><code><span class="token comment">// Load hash from your password DB.</span>
bcrypt<span class="token punctuation">.</span><span class="token function">compare</span><span class="token punctuation">(</span>myPlaintextPassword<span class="token punctuation">,</span> hash<span class="token punctuation">)</span><span class="token punctuation">.</span><span class="token function">then</span><span class="token punctuation">(</span><span class="token keyword">function</span><span class="token punctuation">(</span><span class="token parameter">result</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
    <span class="token comment">// result == true</span>
<span class="token punctuation">}</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
bcrypt<span class="token punctuation">.</span><span class="token function">compare</span><span class="token punctuation">(</span>someOtherPlaintextPassword<span class="token punctuation">,</span> hash<span class="token punctuation">)</span><span class="token punctuation">.</span><span class="token function">then</span><span class="token punctuation">(</span><span class="token keyword">function</span><span class="token punctuation">(</span><span class="token parameter">result</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
    <span class="token comment">// result == false</span>
<span class="token punctuation">}</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
</code></pre></div><p>This is also compatible with <code>async/await</code></p> <div class="language-javascript extra-class"><pre class="language-javascript"><code><span class="token keyword">async</span> <span class="token keyword">function</span> <span class="token function">checkUser</span><span class="token punctuation">(</span><span class="token parameter">username<span class="token punctuation">,</span> password</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
    <span class="token comment">//... fetch user from a db etc.</span>
...
<span class="token punctuation">}</span>
</code></pre></div><h3 id="esm-import"><a href="#esm-import" class="header-anchor">#</a> ESM import</h3> <div class="language-javascript extra-class"><pre class="language-javascript"><code><span class="token keyword">import</span> bcrypt <span class="token keyword">from</span> <span class="token string">&quot;bcrypt&quot;</span><span class="token punctuation">;</span>
...
</code></pre></div><h3 id="sync"><a href="#sync" class="header-anchor">#</a> sync</h3> <div class="language-javascript extra-class"><pre class="language-javascript"><code><span class="token keyword">const</span> bcrypt <span class="token operator">=</span> <span class="token function">require</span><span class="token punctuation">(</span><span class="token string">'bcrypt'</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
<span class="token keyword">const</span> saltRounds <span class="token operator">=</span> <span class="token number">10</span><span class="token punctuation">;</span>
<span class="token keyword">const</span> myPlaintextPassword <span class="token operator">=</span> <span class="token string">'s0/\/\P4$$w0rD'</span><span class="token punctuation">;</span>
<span class="token keyword">const</span> someOtherPlaintextPassword <span class="token operator">=</span> <span class="token string">'not_bacon'</span><span class="token punctuation">;</span>
</code></pre></div><h4 id="to-hash-a-password-2"><a href="#to-hash-a-password-2" class="header-anchor">#</a> To hash a password:</h4> <p>Technique 1 (generate a salt and hash on separate function calls):</p> <div class="language-javascript extra-class"><pre class="language-javascript"><code><span class="token keyword">const</span> salt <span class="token operator">=</span> bcrypt<span class="token punctuation">.</span><span class="token function">genSaltSync</span><span class="token punctuation">(</span>saltRounds<span class="token punctuation">)</span><span class="token punctuation">;</span>
<span class="token keyword">const</span> hash <span class="token operator">=</span> bcrypt<span class="token punctuation">.</span><span class="token function">hashSync</span><span class="token punctuation">(</span>myPlaintextPassword<span class="token punctuation">,</span> salt<span class="token punctuation">)</span><span class="token punctuation">;</span>
<span class="token comment">// Store hash in your password DB.</span>
</code></pre></div><p>Technique 2 (auto-gen a salt and hash):</p> <div class="language-javascript extra-class"><pre class="language-javascript"><code><span class="token keyword">const</span> hash <span class="token operator">=</span> bcrypt<span class="token punctuation">.</span><span class="token function">hashSync</span><span class="token punctuation">(</span>myPlaintextPassword<span class="token punctuation">,</span> saltRounds<span class="token punctuation">)</span><span class="token punctuation">;</span>
<span class="token comment">// Store hash in your password DB.</span>
</code></pre></div><p>As with async, both techniques achieve the same end-result.</p> <h4 id="to-check-a-password-2"><a href="#to-check-a-password-2" class="header-anchor">#</a> To check a password:</h4> <div class="language-javascript extra-class"><pre class="language-javascript"><code><span class="token comment">// Load hash from your password DB.</span>
bcrypt<span class="token punctuation">.</span><span class="token function">compareSync</span><span class="token punctuation">(</span>myPlaintextPassword<span class="token punctuation">,</span> hash<span class="token punctuation">)</span><span class="token punctuation">;</span> <span class="token comment">// true</span>
bcrypt<span class="token punctuation">.</span><span class="token function">compareSync</span><span class="token punctuation">(</span>someOtherPlaintextPassword<span class="token punctuation">,</span> hash<span class="token punctuation">)</span><span class="token punctuation">;</span> <span class="token comment">// false</span>
</code></pre></div><p><a href="#a-note-on-timing-attacks">A Note on Timing Attacks</a></p> <h3 id="why-is-async-mode-recommended-over-sync-mode"><a href="#why-is-async-mode-recommended-over-sync-mode" class="header-anchor">#</a> Why is async mode recommended over sync mode?</h3> <p>We recommend using async API if you use <code>bcrypt</code> on a server. Bcrypt hashing is CPU intensive which will cause the sync APIs to block the event loop and prevent your application from servicing any inbound requests or events. The async version uses a thread pool which does not block the main event loop.</p> <h2 id="api"><a href="#api" class="header-anchor">#</a> API</h2> <p><code>BCrypt.</code></p> <ul><li><code>genSaltSync(rounds, minor)</code> <ul><li><code>rounds</code> - [OPTIONAL] - the cost of processing the data. (default - 10)</li> <li><code>minor</code> - [OPTIONAL] - minor version of bcrypt to use. (default - b)</li></ul></li> <li><code>genSalt(rounds, minor, cb)</code> <ul><li><code>rounds</code> - [OPTIONAL] - the cost of processing the data. (default - 10)</li> <li><code>minor</code> - [OPTIONAL] - minor version of bcrypt to use. (default - b)</li> <li><code>cb</code> - [OPTIONAL] - a callback to be fired once the salt has been generated. uses eio making it asynchronous. If <code>cb</code> is not specified, a <code>Promise</code> is returned if Promise support is available.
<ul><li><code>err</code> - First parameter to the callback detailing any errors.</li> <li><code>salt</code> - Second parameter to the callback providing the generated salt.</li></ul></li></ul></li> <li><code>hashSync(data, salt)</code> <ul><li><code>data</code> - [REQUIRED] - the data to be encrypted.</li> <li><code>salt</code> - [REQUIRED] - the salt to be used to hash the password. if specified as a number then a salt will be generated with the specified number of rounds and used (see example under <strong>Usage</strong>).</li></ul></li> <li><code>hash(data, salt, cb)</code> <ul><li><code>data</code> - [REQUIRED] - the data to be encrypted.</li> <li><code>salt</code> - [REQUIRED] - the salt to be used to hash the password. if specified as a number then a salt will be generated with the specified number of rounds and used (see example under <strong>Usage</strong>).</li> <li><code>cb</code> - [OPTIONAL] - a callback to be fired once the data has been encrypted. uses eio making it asynchronous. If <code>cb</code> is not specified, a <code>Promise</code> is returned if Promise support is available.
<ul><li><code>err</code> - First parameter to the callback detailing any errors.</li> <li><code>encrypted</code> - Second parameter to the callback providing the encrypted form.</li></ul></li></ul></li> <li><code>compareSync(data, encrypted)</code> <ul><li><code>data</code> - [REQUIRED] - data to compare.</li> <li><code>encrypted</code> - [REQUIRED] - data to be compared to.</li></ul></li> <li><code>compare(data, encrypted, cb)</code> <ul><li><code>data</code> - [REQUIRED] - data to compare.</li> <li><code>encrypted</code> - [REQUIRED] - data to be compared to.</li> <li><code>cb</code> - [OPTIONAL] - a callback to be fired once the data has been compared. uses eio making it asynchronous. If <code>cb</code> is not specified, a <code>Promise</code> is returned if Promise support is available.
<ul><li><code>err</code> - First parameter to the callback detailing any errors.</li> <li><code>same</code> - Second parameter to the callback providing whether the data and encrypted forms match [true | false].</li></ul></li></ul></li> <li><code>getRounds(encrypted)</code> - return the number of rounds used to encrypt a given hash
<ul><li><code>encrypted</code> - [REQUIRED] - hash from which the number of rounds used should be extracted.</li></ul></li></ul> <h2 id="a-note-on-rounds"><a href="#a-note-on-rounds" class="header-anchor">#</a> A Note on Rounds</h2> <p>A note about the cost: when you are hashing your data, the module will go through a series of rounds to give you a secure hash. The value you submit is not just the number of rounds the module will go through to hash your data. The module will use the value you enter and go through <code>2^rounds</code> hashing iterations.</p> <p>From @garthk, on a 2GHz core you can roughly expect:</p> <div class="language- extra-class"><pre><code>rounds=8 : ~40 hashes/sec
...
</code></pre></div><h2 id="a-note-on-timing-attacks"><a href="#a-note-on-timing-attacks" class="header-anchor">#</a> A Note on Timing Attacks</h2> <p>Because it's come up multiple times in this project and other bcrypt projects, it needs to be said. The <code>bcrypt</code> library is not susceptible to timing attacks. From codahale/bcrypt-ruby#42:</p> <blockquote><p>One of the desired properties of a cryptographic hash function is preimage attack resistance, which means there is no shortcut for generating a message which, when hashed, produces a specific digest.</p></blockquote> <p>A great thread on this, in much more detail can be found @ codahale/bcrypt-ruby#43</p> <p>If you're unfamiliar with timing attacks and want to learn more you can find a great writeup @ <a href="https://codahale.com/a-lesson-in-timing-attacks/" target="_blank" rel="noopener noreferrer">A Lesson In Timing Attacks<span><svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false" x="0px" y="0px" viewBox="0 0 100 100" width="15" height="15" class="icon outbound"><path fill="currentColor" d="M18.8,85.1h56l0,0c2.2,0,4-1.8,4-4v-32h-8v28h-48v-48h28v-8h-32l0,0c-2.2,0-4,1.8-4,4v56C14.8,83.3,16.6,85.1,18.8,85.1z"></path> <polygon fill="currentColor" points="45.7,48.7 51.3,54.3 77.2,28.5 77.2,37.2 85.2,37.2 85.2,14.9 62.8,14.9 62.8,22.9 71.5,22.9"></polygon></svg> <span class="sr-only">(opens new window)</span></span></a></p> <p>However, timing attacks are real. And the comparison function is <em>not</em> time safe. That means that it may exit the function early in the comparison process. Timing attacks happen because of the above. We don't need to be careful that an attacker will learn anything, and our comparison function provides a comparison of hashes. It is a utility to the overall purpose of the library. If you end up using it for something else, we cannot guarantee the security of the comparator. Keep that in mind as you use the library.</p> <h2 id="hash-info"><a href="#hash-info" class="header-anchor">#</a> Hash Info</h2> <p>The characters that comprise the resultant hash are <code>./ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789$</code>.</p> <p>Resultant hashes will be 60 characters long and they will include the salt among other parameters, as follows:</p> <p><code>$[algorithm]$[cost]$[salt][hash]</code></p> <ul><li>2 chars hash algorithm identifier prefix. <code>&quot;$2a$&quot; or &quot;$2b$&quot;</code> indicates BCrypt</li> <li>Cost-factor (n). Represents the exponent used to determine how many iterations 2^n</li> <li>16-byte (128-bit) salt, base64 encoded to 22 characters</li> <li>24-byte (192-bit) hash, base64 encoded to 31 characters</li></ul> <p>Example:</p> <div class="language- extra-class"><pre class="language-text"><code>$2b$10$nOUIs5kJ7naTuTFkBy1veuK0kSxUFXfuaOKdOKf9xYT0KKIGSJwFa
 |  |  |                     |
...
</code></pre></div><h2 id="testing"><a href="#testing" class="header-anchor">#</a> Testing</h2> <p>If you create a pull request, tests better pass 😃</p> <div class="language- extra-class"><pre class="language-text"><code>npm install
npm test
</code></pre></div><h2 id="credits"><a href="#credits" class="header-anchor">#</a> Credits</h2> <p>The code for this comes from a few sources:</p> <ul><li>blowfish.cc - OpenBSD</li> <li>bcrypt.cc - OpenBSD</li> <li>bcrypt::gen_salt - <a href="http://mail-index.netbsd.org/tech-crypto/2002/05/24/msg000204.html" target="_blank" rel="noopener noreferrer">gen_salt inclusion to bcrypt<span><svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false" x="0px" y="0px" viewBox="0 0 100 100" width="15" height="15" class="icon outbound"><path fill="currentColor" d="M18.8,85.1h56l0,0c2.2,0,4-1.8,4-4v-32h-8v28h-48v-48h28v-8h-32l0,0c-2.2,0-4,1.8-4,4v56C14.8,83.3,16.6,85.1,18.8,85.1z"></path> <polygon fill="currentColor" points="45.7,48.7 51.3,54.3 77.2,28.5 77.2,37.2 85.2,37.2 85.2,14.9 62.8,14.9 62.8,22.9 71.5,22.9"></polygon></svg> <span class="sr-only">(opens new window)</span></span></a></li> <li>bcrypt_node.cc - me</li></ul> <h2 id="contributors"><a href="#contributors" class="header-anchor">#</a> Contributors</h2> <ul><li><a href="https://github.com/Shadowfiend" target="_blank" rel="noopener noreferrer">Antonio Salazar Cardozo<span><svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false" x="0px" y="0px" viewBox="0 0 100 100" width="15" height="15" class="icon outbound"><path fill="currentColor" d="M18.8,85.1h56l0,0c2.2,0,4-1.8,4-4v-32h-8v28h-48v-48h28v-8h-32l0,0c-2.2,0-4,1.8-4,4v56C14.8,83.3,16.6,85.1,18.8,85.1z"></path> <polygon fill="currentColor" points="45.7,48.7 51.3,54.3 77.2,28.5 77.2,37.2 85.2,37.2 85.2,14.9 62.8,14.9 62.8,22.9 71.5,22.9"></polygon></svg> <span class="sr-only">(opens new window)</span></span></a> - Early MacOS X support (when we used libbsd)</li> <li><a href="https://github.com/pixelglow" target="_blank" rel="noopener noreferrer">Ben Glow<span><svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false" x="0px" y="0px" viewBox="0 0 100 100" width="15" height="15" class="icon outbound"><path fill="currentColor" d="M18.8,85.1h56l0,0c2.2,0,4-1.8,4-4v-32h-8v28h-48v-48h28v-8h-32l0,0c-2.2,0-4,1.8-4,4v56C14.8,83.3,16.6,85.1,18.8,85.1z"></path> <polygon fill="currentColor" points="45.7,48.7 51.3,54.3 77.2,28.5 77.2,37.2 85.2,37.2 85.2,14.9 62.8,14.9 62.8,22.9 71.5,22.9"></polygon></svg> <span class="sr-only">(opens new window)</span></span></a> - Fixes for thread safety with async calls</li> <li><a href="https://github.com/thegoleffect" target="_blank" rel="noopener noreferrer">Van Nguyen<span><svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false" x="0px" y="0px" viewBox="0 0 100 100" width="15" height="15" class="icon outbound"><path fill="currentColor" d="M18.8,85.1h56l0,0c2.2,0,4-1.8,4-4v-32h-8v28h-48v-48h28v-8h-32l0,0c-2.2,0-4,1.8-4,4v56C14.8,83.3,16.6,85.1,18.8,85.1z"></path> <polygon fill="currentColor" points="45.7,48.7 51.3,54.3 77.2,28.5 77.2,37.2 85.2,37.2 85.2,14.9 62.8,14.9 62.8,22.9 71.5,22.9"></polygon></svg> <span class="sr-only">(opens new window)</span></span></a> - Found a timing attack in the comparator</li> <li><a href="https://github.com/newitfarmer" target="_blank" rel="noopener noreferrer">NewITFarmer<span><svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false" x="0px" y="0px" viewBox="0 0 100 100" width="15" height="15" class="icon outbound"><path fill="currentColor" d="M18.8,85.1h56l0,0c2.2,0,4-1.8,4-4v-32h-8v28h-48v-48h28v-8h-32l0,0c-2.2,0-4,1.8-4,4v56C14.8,83.3,16.6,85.1,18.8,85.1z"></path> <polygon fill="currentColor" points="45.7,48.7 51.3,54.3 77.2,28.5 77.2,37.2 85.2,37.2 85.2,14.9 62.8,14.9 62.8,22.9 71.5,22.9"></polygon></svg> <span class="sr-only">(opens new window)</span></span></a> - Initial Cygwin support</li> <li><a href="https://github.com/dtrejo" target="_blank" rel="noopener noreferrer">David Trejo<span><svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false" x="0px" y="0px" viewBox="0 0 100 100" width="15" height="15" class="icon outbound"><path fill="currentColor" d="M18.8,85.1h56l0,0c2.2,0,4-1.8,4-4v-32h-8v28h-48v-48h28v-8h-32l0,0c-2.2,0-4,1.8-4,4v56C14.8,83.3,16.6,85.1,18.8,85.1z"></path> <polygon fill="currentColor" points="45.7,48.7 51.3,54.3 77.2,28.5 77.2,37.2 85.2,37.2 85.2,14.9 62.8,14.9 62.8,22.9 71.5,22.9"></polygon></svg> <span class="sr-only">(opens new window)</span></span></a> - packaging fixes</li> <li><a href="https://github.com/alfredwesterveld" target="_blank" rel="noopener noreferrer">Alfred Westerveld<span><svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false" x="0px" y="0px" viewBox="0 0 100 100" width="15" height="15" class="icon outbound"><path fill="currentColor" d="M18.8,85.1h56l0,0c2.2,0,4-1.8,4-4v-32h-8v28h-48v-48h28v-8h-32l0,0c-2.2,0-4,1.8-4,4v56C14.8,83.3,16.6,85.1,18.8,85.1z"></path> <polygon fill="currentColor" points="45.7,48.7 51.3,54.3 77.2,28.5 77.2,37.2 85.2,37.2 85.2,14.9 62.8,14.9 62.8,22.9 71.5,22.9"></polygon></svg> <span class="sr-only">(opens new window)</span></span></a> - packaging fixes</li> <li><a href="https://twitter.com/vincentcr" target="_blank" rel="noopener noreferrer">Vincent Côté-Roy<span><svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false" x="0px" y="0px" viewBox="0 0 100 100" width="15" height="15" class="icon outbound"><path fill="currentColor" d="M18.8,85.1h56l0,0c2.2,0,4-1.8,4-4v-32h-8v28h-48v-48h28v-8h-32l0,0c-2.2,0-4,1.8-4,4v56C14.8,83.3,16.6,85.1,18.8,85.1z"></path> <polygon fill="currentColor" points="45.7,48.7 51.3,54.3 77.2,28.5 77.2,37.2 85.2,37.2 85.2,14.9 62.8,14.9 62.8,22.9 71.5,22.9"></polygon></svg> <span class="sr-only">(opens new window)</span></span></a> - Testing around concurrency issues</li> <li><a href="https://github.com/lloyd" target="_blank" rel="noopener noreferrer">Lloyd Hilaiel<span><svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false" x="0px" y="0px" viewBox="0 0 100 100" width="15" height="15" class="icon outbound"><path fill="currentColor" d="M18.8,85.1h56l0,0c2.2,0,4-1.8,4-4v-32h-8v28h-48v-48h28v-8h-32l0,0c-2.2,0-4,1.8-4,4v56C14.8,83.3,16.6,85.1,18.8,85.1z"></path> <polygon fill="currentColor" points="45.7,48.7 51.3,54.3 77.2,28.5 77.2,37.2 85.2,37.2 85.2,14.9 62.8,14.9 62.8,22.9 71.5,22.9"></polygon></svg> <span class="sr-only">(opens new window)</span></span></a> - Documentation fixes</li> <li><a href="https://github.com/shtylman" target="_blank" rel="noopener noreferrer">Roman Shtylman<span><svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false" x="0px" y="0px" viewBox="0 0 100 100" width="15" height="15" class="icon outbound"><path fill="currentColor" d="M18.8,85.1h56l0,0c2.2,0,4-1.8,4-4v-32h-8v28h-48v-48h28v-8h-32l0,0c-2.2,0-4,1.8-4,4v56C14.8,83.3,16.6,85.1,18.8,85.1z"></path> <polygon fill="currentColor" points="45.7,48.7 51.3,54.3 77.2,28.5 77.2,37.2 85.2,37.2 85.2,14.9 62.8,14.9 62.8,22.9 71.5,22.9"></polygon></svg> <span class="sr-only">(opens new window)</span></span></a> - Code refactoring, general rot reduction, compile options, better memory management with delete and new, and an upgrade to libuv over eio/ev.</li> <li><a href="https://github.com/vadimg" target="_blank" rel="noopener noreferrer">Vadim Graboys<span><svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false" x="0px" y="0px" viewBox="0 0 100 100" width="15" height="15" class="icon outbound"><path fill="currentColor" d="M18.8,85.1h56l0,0c2.2,0,4-1.8,4-4v-32h-8v28h-48v-48h28v-8h-32l0,0c-2.2,0-4,1.8-4,4v56C14.8,83.3,16.6,85.1,18.8,85.1z"></path> <polygon fill="currentColor" points="45.7,48.7 51.3,54.3 77.2,28.5 77.2,37.2 85.2,37.2 85.2,14.9 62.8,14.9 62.8,22.9 71.5,22.9"></polygon></svg> <span class="sr-only">(opens new window)</span></span></a> - Code changes to support 0.5.5+</li> <li><a href="https://github.com/bnoordhuis" target="_blank" rel="noopener noreferrer">Ben Noordhuis<span><svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false" x="0px" y="0px" viewBox="0 0 100 100" width="15" height="15" class="icon outbound"><path fill="currentColor" d="M18.8,85.1h56l0,0c2.2,0,4-1.8,4-4v-32h-8v28h-48v-48h28v-8h-32l0,0c-2.2,0-4,1.8-4,4v56C14.8,83.3,16.6,85.1,18.8,85.1z"></path> <polygon fill="currentColor" points="45.7,48.7 51.3,54.3 77.2,28.5 77.2,37.2 85.2,37.2 85.2,14.9 62.8,14.9 62.8,22.9 71.5,22.9"></polygon></svg> <span class="sr-only">(opens new window)</span></span></a> - Fixed a thread safety issue in nodejs that was perfectly mappable to this module.</li> <li><a href="https://github.com/tootallnate" target="_blank" rel="noopener noreferrer">Nate Rajlich<span><svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false" x="0px" y="0px" viewBox="0 0 100 100" width="15" height="15" class="icon outbound"><path fill="currentColor" d="M18.8,85.1h56l0,0c2.2,0,4-1.8,4-4v-32h-8v28h-48v-48h28v-8h-32l0,0c-2.2,0-4,1.8-4,4v56C14.8,83.3,16.6,85.1,18.8,85.1z"></path> <polygon fill="currentColor" points="45.7,48.7 51.3,54.3 77.2,28.5 77.2,37.2 85.2,37.2 85.2,14.9 62.8,14.9 62.8,22.9 71.5,22.9"></polygon></svg> <span class="sr-only">(opens new window)</span></span></a> - Bindings and build process.</li> <li><a href="https://github.com/seanmonstar" target="_blank" rel="noopener noreferrer">Sean McArthur<span><svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false" x="0px" y="0px" viewBox="0 0 100 100" width="15" height="15" class="icon outbound"><path fill="currentColor" d="M18.8,85.1h56l0,0c2.2,0,4-1.8,4-4v-32h-8v28h-48v-48h28v-8h-32l0,0c-2.2,0-4,1.8-4,4v56C14.8,83.3,16.6,85.1,18.8,85.1z"></path> <polygon fill="currentColor" points="45.7,48.7 51.3,54.3 77.2,28.5 77.2,37.2 85.2,37.2 85.2,14.9 62.8,14.9 62.8,22.9 71.5,22.9"></polygon></svg> <span class="sr-only">(opens new window)</span></span></a> - Windows Support</li> <li><a href="https://github.com/weareu" target="_blank" rel="noopener noreferrer">Fanie Oosthuysen<span><svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false" x="0px" y="0px" viewBox="0 0 100 100" width="15" height="15" class="icon outbound"><path fill="currentColor" d="M18.8,85.1h56l0,0c2.2,0,4-1.8,4-4v-32h-8v28h-48v-48h28v-8h-32l0,0c-2.2,0-4,1.8-4,4v56C14.8,83.3,16.6,85.1,18.8,85.1z"></path> <polygon fill="currentColor" points="45.7,48.7 51.3,54.3 77.2,28.5 77.2,37.2 85.2,37.2 85.2,14.9 62.8,14.9 62.8,22.9 71.5,22.9"></polygon></svg> <span class="sr-only">(opens new window)</span></span></a> - Windows Support</li> <li><a href="https://github.com/recrsn" target="_blank" rel="noopener noreferrer">Amitosh Swain Mahapatra<span><svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false" x="0px" y="0px" viewBox="0 0 100 100" width="15" height="15" class="icon outbound"><path fill="currentColor" d="M18.8,85.1h56l0,0c2.2,0,4-1.8,4-4v-32h-8v28h-48v-48h28v-8h-32l0,0c-2.2,0-4,1.8-4,4v56C14.8,83.3,16.6,85.1,18.8,85.1z"></path> <polygon fill="currentColor" points="45.7,48.7 51.3,54.3 77.2,28.5 77.2,37.2 85.2,37.2 85.2,14.9 62.8,14.9 62.8,22.9 71.5,22.9"></polygon></svg> <span class="sr-only">(opens new window)</span></span></a> - <span class="katex"><span class="katex-mathml"><math><semantics><mrow><mn>2</mn><mi>b</mi></mrow><annotation encoding="application/x-tex">2b</annotation></semantics></math></span><span aria-hidden="true" class="katex-html"><span class="strut" style="height:0.69444em;"></span><span class="strut bottom" style="height:0.69444em;vertical-align:0em;"></span><span class="base textstyle uncramped"><span class="mord mathrm">2</span><span class="mord mathit">b</span></span></span></span> hash support, ES6 Promise support</li> <li><a href="https://github.com/NickNaso" target="_blank" rel="noopener noreferrer">Nicola Del Gobbo<span><svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false" x="0px" y="0px" viewBox="0 0 100 100" width="15" height="15" class="icon outbound"><path fill="currentColor" d="M18.8,85.1h56l0,0c2.2,0,4-1.8,4-4v-32h-8v28h-48v-48h28v-8h-32l0,0c-2.2,0-4,1.8-4,4v56C14.8,83.3,16.6,85.1,18.8,85.1z"></path> <polygon fill="currentColor" points="45.7,48.7 51.3,54.3 77.2,28.5 77.2,37.2 85.2,37.2 85.2,14.9 62.8,14.9 62.8,22.9 71.5,22.9"></polygon></svg> <span class="sr-only">(opens new window)</span></span></a> - Initial implementation with N-API</li></ul> <h2 id="license"><a href="#license" class="header-anchor">#</a> License</h2> <p>Unless stated elsewhere, file headers or otherwise, the license as stated in the LICENSE file.</p></div> <footer class="page-edit"><!----> <!----></footer> <!----> </main></div><div class="global-ui"><!----></div></div>
    <script src="/edu_db_labs/assets/js/app.b4446a62.js" defer></script><script src="/edu_db_labs/assets/js/2.c940f4cf.js" defer></script><script src="/edu_db_labs/assets/js/1.444b6f58.js" defer></script><script src="/edu_db_labs/assets/js/65.ca6931ec.js" defer></script>
  </body>
</html>
